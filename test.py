...
def search_category_in_catalog(url: str, catalog_list: list) -> dict:
    ...
    print(url.split('https://www.wildberries.ru')[-1])
    for catalog in catalog_list:
        if url.split('https://www.wildberries.ru')[-1] == catalog['url']:
            print(catalog['url'])
...
def parser(url: str, low_price: int = 1, top_price: int = 1000000, discount: int = 0):
    ...
    try:
        ...
        # print(category)
        data_list = []
        for page in range(1, 101):  # вб отдает 50 страниц товара
            data = scrap_page(
                page=page,
                ...
                discount=discount)
            # print(data)
            print(len(get_data_from_json(data)))
...
if __name__ == '__main__':
    ...
    url='https://www.wildberries.ru/catalog/tovary-dlya-zhivotnyh/dlya-koshek/lotki-i-napolniteli'
